...
def build_env_cmd(conda_env, exe):
    ...
    if conda_env and current_env.lower() == conda_env.lower():
        ...
        if found:
            return [found]
        sibling_exe = Path(sys.executable).parent / (exe + ".exe")
...
def rectangular_train(args):
    return args.train_task == "detect" and args.img_width != args.img_height


def train_imgsz_arg(args):
    return max(args.img_width, args.img_height)
...
param(
    ...
    [Parameter(Mandatory=$true)][int]$Patience,
...
$ImgSize = [Math]::Max($ImgWidth, $ImgHeight)
$AmpEnabled = if ($TrainDevice -eq "cuda" -and !($TrainTask -eq "detect" -and $ImgWidth -ne $ImgHeight)) { "True" } else { "False" }
$TrainArgv = @(
    $TrainTask, "train",
    "model=$BaseModel",
    $DataArg,
...
if ($TrainTask -eq "detect" -and $ImgWidth -ne $ImgHeight) {
    $TrainArgv += "rect=True"
}
Invoke-Argv $YoloCmd $TrainArgv
$TrainExitCode = $LASTEXITCODE
if ($TrainExitCode -ne 0) { Write-Host "TRAIN_PROCESS_EXIT_CODE=$TrainExitCode" }
...
def train_local(args, dataset_path: Path, work: Path):
    ...
    train_runner = Path(__file__).with_name("ultralytics_train_runner.py")
    ...
    train_args = [
        ...
        f"amp={'True' if str(args.train_device).lower().startswith('cuda') and not rectangular_train(args) else 'False'}",
        f"device={args.train_device}",
        "cos_lr=True",
        "deterministic=False",
    ]
    if args.train_task == "detect":
        train_args.append(f"imgsz={train_imgsz_arg(args)}")
        if rectangular_train(args):
            train_args.append("rect=True")
    else:
        train_args.append(f"imgsz={train_imgsz_arg(args)}")
    train_code = run_train_command(py_cmd + [train_runner] + train_args, stop_signal=stop_signal)
...
def _prepared_names(data):
    raw = data.get("names", [])
    if isinstance(raw, dict):
        def key_fn(item):
            key = str(item[0])
            return (int(key), "") if key.isdigit() else (10**9, key)
        return [str(value) for _, value in sorted(raw.items(), key=key_fn)]
    return [str(value) for value in raw] if isinstance(raw, (list, tuple)) else []


def _prepared_split_dir(root: Path, yaml_path: Path, split: str, raw_value):
    ...
    if text:
        ...
        if path.is_absolute():
            ...
            candidates.extend((yaml_path.parent / path, root / path))
    ...
    for candidate in candidates:
        ...
        if candidate.is_dir():
            return candidate
    raise SystemExit(f"Prepared YOLO dataset is missing the {split} images directory")


def prepare_existing_yolo(dataset_yaml: Path, out: Path, copy_dataset: bool = False):
    dataset_yaml = dataset_yaml.expanduser().resolve()
    if not dataset_yaml.is_file():
        raise SystemExit(f"Prepared YOLO dataset YAML not found: {dataset_yaml}")
    data = yaml.safe_load(dataset_yaml.read_text(encoding="utf-8-sig")) or {}
    if not isinstance(data, dict):
        raise SystemExit(f"Invalid YOLO dataset YAML: {dataset_yaml}")
    root = dataset_yaml.parent
    names = _prepared_names(data)
    if not names:
        raise SystemExit("Prepared YOLO dataset has no class names")
    split_dirs = {}
    for split in ("train", "val", "test"):
        try:
            split_dirs[split] = _prepared_split_dir(root, dataset_yaml, split, data.get(split))
        except SystemExit:
            if split == "test":
                continue
            raise
    ...
    normalized = {"names": {index: name for index, name in enumerate(names)}}
    if copy_dataset:
        for split, images_dir in split_dirs.items():
            labels_dir = images_dir.parent / "labels"
            if not labels_dir.is_dir():
                raise SystemExit(f"Prepared YOLO labels directory not found: {labels_dir}")
            shutil.copytree(images_dir, out / "images" / split, dirs_exist_ok=True)
            shutil.copytree(labels_dir, out / "labels" / split, dirs_exist_ok=True)
            normalized[split] = f"images/{split}"
        normalized["path"] = "."
    else:
        normalized["path"] = str(root).replace("\\", "/")
        for split, images_dir in split_dirs.items():
            normalized[split] = str(images_dir).replace("\\", "/")
    normalized_yaml = out / "dataset.yaml"
    normalized_yaml.write_text(yaml.safe_dump(normalized, allow_unicode=True, sort_keys=False), encoding="utf-8")
    (out / "classes.txt").write_text("\n".join(names) + "\n", encoding="utf-8")
    train_images = sorted(
...
def run_train_stage(args, script_root: Path):
    ...
    if args.dataset_yaml:
        if args.train_task != "detect":
            raise SystemExit("--dataset-yaml currently supports YOLO detection datasets only")
        dataset_path, train_images = prepare_existing_yolo(
            Path(args.dataset_yaml), yolo_data,
            copy_dataset=args.train_mode == "remote-windows",
        )
        print(f"PREPARED_DATASET_YAML={dataset_path}", flush=True)
        print("PREPARED_DATASET_SPLIT=preserved", flush=True)
    elif args.train_task == "classify":
    ...
    if args.train_mode == "remote-windows":
        if not args.remote_train_user.strip():
            raise SystemExit("--remote-train-user is required when --train-mode remote-windows")
        best_pt, best_onnx = train_remote_windows(args, yolo_data, work, timestamp)
    else:
...
def run_convert_stage(args, script_root: Path, train_result=None):
    ...
    print(f"Uploading to {remote}:{args.vm_work_dir} ...")
    run(["ssh", remote, f"mkdir -p {args.vm_work_dir}"])
    ...
    remote_cmd = (
        f"cd {args.vm_work_dir} && rm -rf {remote_job} {remote_outputs} {remote_outputs}.tar.gz "
        f"&& unzip -o {zip_name} -d {remote_job} "
        f"&& {env_prefix}bash {remote_job}/vm_convert_pack.sh {remote_job} {timestamp}"
    )
...
def build_parser(script_root: Path):
    ...
    ap.add_argument("--patience", type=int, default=0, help="0 表示关闭自动早停，只允许手动停止")
