...
@media(max-width:700px){.pipeline{grid-template-columns:1fr 1fr}.dashboard-focus h3{font-size:20px}.train-primary-bar{bottom:0}.nav-group{display:none}}
</style>
</head>
<body>
<div class="wrap">
...
function showTab(name){if(name==='label'&&!document.body.classList.contains('show-experimental'))name='overview';const target=document.getElementById('tab-'+name); if(!target) name='overview'; document.querySelectorAll('.tab').forEach(x=>x.classList.remove('active'));document.getElementById('tab-'+name).classList.add('active');let activeButton=null;document.querySelectorAll('.nav button[data-tab]').forEach(x=>{const active=x.dataset.tab===name;x.classList.toggle('active',active);if(active){x.setAttribute('aria-current','page');activeButton=x}else x.removeAttribute('aria-current')});const pageTitle=document.querySelector('#tab-'+name+'>h2')?.textContent?.trim();document.title=pageTitle?`${pageTitle} · YOLO团队训练平台`:'YOLO团队训练平台';localStorage.setItem('yoloTeamPlatformTab',name);window.scrollTo({top:0,behavior:'auto'});requestAnimationFrame(()=>{if(activeButton&&matchMedia('(max-width:980px)').matches)activeButton.scrollIntoView({block:'nearest',inline:'center',behavior:'smooth'})});if(name==='overview')refreshDashboard();if(name==='projects') loadProjects(); if(name==='models') loadBaseModels(); if(name==='label') loadLabelResults(); if(name==='assets') loadModelAssets(); if(name==='collab') loadAnnotationService()}
