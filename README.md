...
1. 双击 `YOLO-Team-Training-Platform-Setup-v3.2.19-beta.exe`。
...
工作台总览包含“软件更新”区域：显示当前版本、最新版本、发布时间和完整更新内容。平台每 6 小时最多自动检查一次，也可手动检查；桌面版可下载、校验并确认安装，源码浏览器模式会打开官方 Release 下载页。检查失败不会影响离线标注、训练或导出。
...
| `host_train_export.py`、`export_model.py`、`rdk_x5_deployment.py`、`rdk_x5_remote.py` | 训练、多设备导出、RDK X5 NPU 转换包及 WSL/SSH 部署 |
| `tests/` | 自动测试 |
...
不再强制依赖 Roboflow。“项目中心”的目标检测项目会自动同步到个人/团队标注中心；默认“待处理”队列会自动恢复未完成任务，选择类别、拖动画框，再点击“完成并下一张”即可。页面会静默自动保存，数字键可切换类别，直接点击已有框即可编辑；没有目标的图片使用“空图并下一张”作为负样本完成。团队成员可以直接从公共队列自动领取下一张，不必预先分配；审核默认关闭，需要双人复核时再按项目开启。“项目与团队”默认只展示当前项目和高频流程，成员、任务、导出及危险操作按需展开。管理员可以通过完整名称确认删除协作标注副本，操作不会连带删除训练平台数据集或已导出压缩包。已完成数据可导出 Ultralytics YOLO、COCO、VOC 或 LabelMe。详见[协作标注文档](docs/COLLABORATIVE_ANNOTATION.md)。
...
导出页会直接显示“训练 → 导出/转换 → 校验 → 板端运行”的连续步骤。RDK X5 会打包原始权重、中间 ONNX、20–50 张校准图片、隔离环境脚本和官方转换入口；随后可直接在页面检查/配置本机 Ubuntu 22.04 WSL2、编译 Bayes-e `.bin`，再通过 SSH 上传到官方 RDK 系统做模型加载、性能与可选图片推理。全程不需要 Docker，板卡也不承担编译。页面不会把 ONNX 误报成 NPU 已完成；任何 INT8 模型都要在目标设备上重新验证精度。
...
rdk_x5_remote.py         本机 WSL 编译与 RDK X5 SSH 真机验证
```
